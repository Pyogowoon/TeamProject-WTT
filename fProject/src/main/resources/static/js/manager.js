...
mDetail.insertR = function(frm){
	...
	$.post("m_detail_insertR",param,function(e){
		/*
		let posters = $('.dPoster').get();
		let vids = $('.dVid').get();
		let posterArray=[];
		let vidArray=[];
		let snoArray=[];
		for(let i=0; i<posters.length ; i++){
			posterArray.push({"poster" : posters[i].files[0]});
			vidArray.push({"vid" : vids[i].files[0]});
			snoArray.push({"sno" : e[i]});
		}
		if(posters.length>0){
		}
		*/
		...
		//let frmAtt = new FormData();
		//frmAtt.append(posterArray);
		//frmAtt.append(vidArray);
		//frmAtt.append(snoArray);
		...
		$.ajax({
			...
			success : function(){ 
				...
				frm.submit();
				/*
				let modifyData = $('.modifyData').get();
				console.log(modifyData.length);
				sfrm = $('#frm_temp')[0];
				frmAtt = new FormData();
				for(let i=0 ; i<modifyData.length ; i++){
					let dpOriFile = $('.dpOriFile').eq(i).val();
					let dpSysFiletag = $('.dpSysfile').eq(i)[0];	
					let dpSysFile = $('.dpSysFile').eq(i).val();
					let changePoster = $('.changePoster').eq(i).val();
					let vOriFile = $('.vOriFile').eq(i).val();	
					let vSysFile = $('.vSysFile').eq(i).val();
					let vSysFiletag = $('.vSysFile').eq(i)[0];
					let changeVid = $('.changeVid').eq(i).val();
					let vSno = $('.vSno').eq(i)[0];
					let modifyP = $('.modifyP').eq(i)[0];
					let modifyV = $('.modifyV').eq(i)[0];
					console.log(modifyP);
					console.log(modifyV);
					console.log(vSno);
					if(dpOriFile != changePoster){
						mDetail.postercon = 1;
					}
					if(vOriFile != changeVid){
						mDetail.vidcon = 1;
					}
					
					if(mDetail.postercon==1 && mDetail.vidcon ==1 ){
						sfrm.append(modifyP);
						sfrm.append(dpSysFiletag);
						sfrm.append(modifyV);
						sfrm.append(vSysFiletag);
						sfrm.append(vSno);
					}else if(mDetail.postercon==1 && mDetail.vidcon == -1){
						frmAtt.append('mdPoster',modifyP.files[0]);
						frmAtt.append('deleteP',dpSysFile);
						frmAtt.append('mdVid','');
						frmAtt.append('deleteV','');
						frmAtt.append('sno',vSno);
					}else if(mDetail.postercon ==-1 && mDetail.vidcon == 1){
						frmAtt.append('mdPoster','');
						frmAtt.append('deleteP','');
						frmAtt.append('mdVid',modifyV.files[0]);
						frmAtt.append('deleteV',vSysFile);
						frmAtt.append('sno',vSno);
					}
